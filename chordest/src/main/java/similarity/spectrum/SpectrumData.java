...
//		builder.append(wavFilePath, other.wavFilePath);
	...
	public chordest.spectrum.SpectrumData toNewSpectrumData() {
		chordest.spectrum.SpectrumData result = new chordest.spectrum.SpectrumData();
		result.beatTimes = this.beatTimes;
		result.f0 = this.f0;
		result.samplingRate = this.samplingRate;
		result.scaleInfo = this.scaleInfo.toNewScaleInfo();
		result.spectrum = this.spectrum;
		result.startNoteOffsetInSemitonesFromF0 = this.startNoteOffsetInSemitonesFromF0;
		result.totalSeconds = this.totalSeconds;
		result.wavFilePath = this.wavFilePath;
		return result;
	}
