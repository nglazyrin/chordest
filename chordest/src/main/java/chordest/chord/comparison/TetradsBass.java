...
public class TetradsBass implements IEvaluationMetric {
	...
	public Chord map(Chord chord) {
		...
		for (String shorthand : shorthands) {
			int bassInterval = (chord.getBass().offsetFrom(root) + 12) % 12;
			Chord pattern = new Chord(bassInterval, root, shorthand);
	...
	public double score(Chord reference, Chord estimated) {
		...
		if (! reference.hasSameBassWith(estimated)) {
			return 0;
		}
		Arrays.sort(notes1);
		Arrays.sort(notes2);
