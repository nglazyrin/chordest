# -*- coding: utf-8 -*-
"""
Created on Sat Feb 16 00:05:49 2013
...
#                                         dtype=theano.config.floatX) * input
    ...
    def autoencode(self, v):
        """ This function applies the autoencoder to a vector """
        
        v1 = theano.shared(numpy.asarray(v, dtype=theano.config.floatX),
                                 borrow=True)
        x = T.vector('x')
        y = self.get_hidden_values(x)
        z = self.get_reconstructed_input(y)
        result = theano.function([], z, givens={x: v1})
        return result()

    def autoencode_m(self, m):
        """ This function applies the autoencoder to a matrix """
        
        m1 = theano.shared(numpy.asmatrix(m, dtype=theano.config.floatX),
                                 borrow=True)
        x = T.matrix('x')
        y = self.get_hidden_values(x)
        z = self.get_reconstructed_input(y)
        result = theano.function([], z, givens={x: m1})
        return result()

    def encode_m(self, m):
        """ This function encodes the matrix without decoding """
        
        m1 = theano.shared(numpy.asmatrix(m, dtype=theano.config.floatX),
                                 borrow=True)
        x = T.matrix('x')
        y = self.get_hidden_values(x)
        result = theano.function([], y, givens={x: m1})
        return result()
